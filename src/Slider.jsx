...
function Slider() {
  ...
  useEffect(() => {
    const nextSlide = () => {
      setSlideIndex((prevIndex) => (prevIndex + 1) % 3); // Loop between 0, 1, and 2
    };

    const id = setInterval(nextSlide, 5000); // Change slide every 5 secondas

    return () => clearInterval(id); // Cleanup the interval when the component unmounts
  }, []);
  ...
  return (
    <div className='mt-16 parent'>
      ...
      <div className='talks'>
        ...
        <div className={slideIndex === 2 ? 'displaySlide' : ''}>
          <h3><span>"</span> I appreciate the effort the teachers put in developing these children. <span>"</span></h3>
          <p>Mr Joseph</p>
        </div>
      </div>
    </div>
